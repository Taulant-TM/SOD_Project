# 🧠 Salient Object Detection (SOD) — Deep Learning Project

This project implements a **Salient Object Detection (SOD)** system using a
custom TensorFlow/Keras CNN model. The pipeline includes:

✔ Dataset preparation  
✔ Preprocessing & augmentation  
✔ Custom SOD segmentation model  
✔ Custom loss (BCE + Dice)  
✔ IoU & F1 metrics  
✔ Full training loop with checkpoints  
✔ Evaluation script  
✔ Gradio real-time demo  
✔ Clean project structure  

---

## 📂 Project Structure

SOD_Project/
│
├── src/
│ ├── data_loader.py # Loads images/masks, preprocessing, tf.data pipeline
│ ├── prepare_dataset.py # Generates dataset.npz from raw images/masks
│ ├── sod_model.py # Model architecture, loss, metrics
│ ├── train.py # Training loop with checkpoints & logging
│ ├── evaluate.py # Evaluate test set IoU/F1
│ ├── demo_gradio.py # Web demo for inference
│
├── scripts/
│ └── check_data.py # Checks image–mask mismatches & corrupted files
│
├── data/
│ ├── DUTS-TR/ # Raw images 
│ ├── DUTS-TE/ # Raw masks 
│ └── dataset.npz # Prepared dataset generated by prepare_dataset.py
│
├── outputs/
│ ├── checkpoints/ # Saved ckpt-*.index files
│ └── best_model.keras # Best model saved during training
│
├── requirements.txt
└── README.md

---

## 🛠 Installation

```bash
git clone https://github.com/yourusername/SOD_Project.git
cd SOD_Project
python -m venv .venv
source .venv/bin/activate   # Windows: .venv\Scripts\activate
pip install -r requirements.txt
🗂 Prepare the Dataset
If you have raw images & masks under:


data/DUTS-TR/
data/DUTS-TE/
Run:
python src/prepare_dataset.py
This generates:
data/dataset.npz

The split is:
70% train
15% validation
15% test

🏋️ Train the Model
python src/train.py

This script:
Loads dataset.npz
Builds a tf.data pipeline
Trains the SOD CNN
Saves checkpoints to outputs/checkpoints/
Saves the best model to outputs/best_model.keras

📊 Evaluate on Test Set
python src/evaluate.py

Outputs:
IoU
F1 Score
Loss

🌐 Run the Web Demo (Gradio)
python src/demo_gradio.py

Then open:
http://localhost:7860
Upload an image and see the predicted saliency map.

👨‍💻 Author
Taulant Mehana